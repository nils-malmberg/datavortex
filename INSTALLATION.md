...
| « Réseau de neurones » indisponible (« TensorFlow est installé mais Windows refuse de charger sa bibliothèque native ») alors que tout le reste fonctionne | Poste d'entreprise : runtime Visual C++ absent, AppLocker, ou VM sans AVX. Diagnostic dans l'onglet ML et dans `GET /api/ml/capabilities` ; pistes et contournement (`datavortex --no-tensorflow`) dans [backend/CORPORATE_SETUP.md](backend/CORPORATE_SETUP.md). |

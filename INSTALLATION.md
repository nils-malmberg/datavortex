...
| `No solution found... has no wheels with a matching Python ABI tag (e.g., cp314)` | `uv` a choisi automatiquement une version de Python trop récente (DataVortex supporte 3.10 et 3.11, cf. `requires-python`). `uv` doit éviter ce cas de lui-même ; si ça se reproduit, forcez explicitement une version compatible : `uv tool install ./datavortex-cli --python 3.11`. |
| Miroir PyPI interne, proxy TLS d'entreprise, poste sans réseau | Voir [backend/COMPATIBILITY.md](backend/COMPATIBILITY.md), section « Installation sur un réseau d'entreprise » : `UV_INDEX_URL`, `--system-certs`, installation hors ligne depuis un dossier de wheels. |
| « Réseau de neurones » indisponible (« TensorFlow 2.20 est installé mais Windows refuse de charger sa bibliothèque native ») alors que tout le reste fonctionne | Installation faite avec une v1.2.3 / v1.2.4, qui prenaient TensorFlow 2.20 : ses wheels Windows exigent un runtime Visual C++ 2022 récent. Réinstallez (`uv tool install --force ./datavortex-cli`) : depuis la 1.2.5, Windows reçoit la 2.15, qui se charge (détail dans [backend/COMPATIBILITY.md](backend/COMPATIBILITY.md), « TensorFlow par plateforme »). |

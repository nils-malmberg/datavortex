...
| Miroir PyPI interne, proxy TLS d'entreprise, poste sans réseau | Voir [backend/CORPORATE_SETUP.md](backend/CORPORATE_SETUP.md) : `UV_INDEX_URL`, `--system-certs`, installation hors ligne depuis un dossier de wheels. |
| « Réseau de neurones » indisponible (« TensorFlow 2.20 est installé mais Windows refuse de charger sa bibliothèque native ») alors que tout le reste fonctionne | Installation faite avec une v1.2.3 / v1.2.4, qui prenaient TensorFlow 2.20 : ses wheels Windows exigent un runtime Visual C++ 2022 récent. Réinstallez (`uv tool install --force ./datavortex-cli`) : depuis la 1.2.5, Windows reçoit la 2.15, qui se charge. Détail dans [backend/CORPORATE_SETUP.md](backend/CORPORATE_SETUP.md). |

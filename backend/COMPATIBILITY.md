# Matrice de compatibilité — DataVortex v1.2.3
...
| 3.12 | ✅ testé | nécessite TensorFlow ≥ 2.16 (premiers wheels cp312) |
| 3.13 | ❌ exclu | `requires-python = "<3.13"` ; TensorFlow 2.20 le supporte, mais ni les planchers ni le reste de la matrice n'ont été vérifiés |
...
résout aujourd'hui, exécuté sur Python 3.10, 3.11 et 3.12.
...
| numpy | `>=1.24.0,<3.0.0` | 1.24.0 | 2.2.6 (3.10) · 2.4.6 (3.11) · 2.5.3 (3.12) | NumPy 2 passe la suite ; le plafond est la prochaine majeure |
| scipy | `>=1.10.0,<1.19.0` | 1.10.0 | 1.15.3 (3.10) · 1.17.1 (3.11) · 1.18.1 (3.12) | **1.19 retirera `critical_values` du résultat d'`anderson`**, utilisé par les tests de normalité (avertissement depuis 1.17) |
...
| scikit-learn | `>=1.2.0,<2.0.0` | 1.2.0 | 1.7.2 (3.10) · 1.9.1 (3.11, 3.12) | 1.6+ interroge `__sklearn_tags__` — corrigé côté code (adaptateur Keras) |
| matplotlib | `>=3.7.0,<4.0.0` | 3.7.0 | 3.10.9 (3.10) · 3.11.1 (3.11, 3.12) | |
...
| tensorflow-cpu | `>=2.15.0,<2.21.0` | 2.15.0 (Keras 2) | 2.20.0 (Keras 3) | voir la section TensorFlow |
...
dépend de la plateforme — les marqueurs de `pyproject.toml` s'en chargent :

| Plateforme | Paquet | Intervalle | Raison |
|---|---|---|---|
| Linux x86_64, Windows AMD64 | `tensorflow-cpu` | `>=2.15,<2.21` | wheels légers (sans CUDA) |
...
La suite a été exécutée avec Keras 2 (TF 2.15) et Keras 3 (TF 2.20) : le
réseau de neurones, l'export TFLite et l'importance par permutation passent
dans les deux cas.

## Ce que garantit la CI

- `backend` (matrice 3.10 / 3.11 / 3.12) : le lockfile — donc les versions
  les plus récentes des intervalles — sur chaque interpréteur supporté.

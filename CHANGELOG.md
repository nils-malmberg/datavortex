...
- **Le bouton « Entraîner le réseau » restait sur « Vérification de TensorFlow… »** le temps que le serveur importe TensorFlow (jusqu'à une minute sur un poste avec antivirus) : la sonde à l'ouverture, ajoutée en 1.2.4, est retirée. L'import est payé au premier entraînement, et si TensorFlow ne se charge pas, le 503 porte le diagnostic. `GET /api/ml/capabilities` reste pour les scripts.
- **Terminal inondé d'avertissements** : `Could not infer format…` (pandas, à chaque détection de type de colonne — le repli sur dateutil est précisément ce qu'on demande), `FutureWarning` SciPy sur `anderson` (on lit `critical_values`, et scipy est plafonné <1.19 pour ça), et les messages d'information/dépréciation de TensorFlow et Keras (`TF_CPP_MIN_LOG_LEVEL=2` par défaut, journal TensorFlow au niveau erreur). Aucun n'était actionnable ; le comportement est inchangé.

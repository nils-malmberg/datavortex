    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <label className="flex flex-1 flex-col gap-1 text-sm">
          <span className="font-medium text-slate-600">Nom de la nouvelle colonne</span>
          <input
            type="text"
            value={name}
      ...
      <label className="flex flex-col gap-1 text-sm">
        <span className="font-medium text-slate-600">Formule</span>
        <textarea
          ref={textareaRef}
          value={formula}
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
        <p className="mb-1.5 font-medium text-slate-700">Fonctions et opérateurs disponibles</p>
      ...
      {previewError && (
        <p className="rounded-md bg-red-50 px-4 py-2 text-sm text-red-700">{previewError}</p>
      )}
      ...
      {preview && !previewError && (
        <div className="overflow-x-auto rounded-lg border border-slate-200">
              ...
              {preview.preview.map((val, i) => (
                <tr key={i}>
                  <td className="px-3 py-1 text-slate-700">
                    {val === null ? <span className="italic text-slate-400">null</span> : String(val)}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {preview.error_count > 0 && (
            <p className="border-t border-slate-200 bg-orange-50 px-3 py-1.5 text-xs text-orange-700">
      ...
      {saveError && <p className="rounded-md bg-red-50 px-4 py-2 text-sm text-red-700">{saveError}</p>}
    </div>
  )
}

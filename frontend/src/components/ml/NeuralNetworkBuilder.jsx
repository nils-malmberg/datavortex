...
export default function NeuralNetworkBuilder({ sessionId, columns, columnTypes, onAddToReport }) {
  ...
  // Pas de sonde TensorFlow à l'ouverture : l'import côté serveur peut prendre
  // une minute (antivirus sur ~500 Mo au premier chargement) et bloquait le
  // bouton pendant ce temps. On paie cet import au premier entraînement, et
  // si TensorFlow ne se charge pas, le serveur répond 503 avec le diagnostic.
  const [tensorflowUnavailable, setTensorflowUnavailable] = useState(false)
  ...
  const handleTrain = async () => {
    setIsTraining(true)
    setError(null)
    setTensorflowUnavailable(false)
    setResult(null)
    try {
      ...
      setTensorflowUnavailable(err?.response?.data?.error?.code === 'TENSORFLOW_UNAVAILABLE')
    ...
    <div className="flex flex-col gap-4">
      <Panel className="flex flex-col gap-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Le premier entraînement charge TensorFlow : comptez de quelques secondes à une minute selon le poste. Les suivants sont immédiats.
        </p>
      </Panel>

      {error && tensorflowUnavailable ? (
        <div
          role="alert"
